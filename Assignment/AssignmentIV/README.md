...
#### 結果與分析
####（一）a-1 與 a-2 雜湊函數分析
#### a-1 與 a-2 在三種不同的 table size 下，21–30 與 51–60 兩組 key 對應到完全相同的 index。此外，a-2 僅是 a-1 的位移變化，本質上仍具有高度規律性，因此雜湊品質不佳，不適合實務應用。
#### （二）b 雜湊函數分析
#### 顯示 21–30 與 51–60 仍然對應到相同的 index，表示 key 的高位數並未有效參與雜湊運算。
#### （三）c-1 與 c-2 雜湊函數分析
#### 21–30 與 51–60 多數情況下會被映射至不同的 index，碰撞明顯減少，顯示該類雜湊函數能同時利用 key 的高位與低位資訊，使分佈更為均勻。
#### 結論
#### a-1 與 a-2 雜湊函數具有高度規律性，碰撞率高，雜湊品質最差。
#### b 雜湊函數雖稍有改善，但仍無法有效區分不同 key 群組。
#### c-1 與 c-2 雜湊函數能較平均地分佈 key，碰撞明顯減少，其中在 table size 為質數時效果最佳。
#### 選擇合適的雜湊函數並搭配質數大小的 hash table，可有效提升雜湊效能。
